...
- Any detected failure (unsupported die type, out-of-range value) or uncaught error (pcall at the dispatch in `handleDiceVisionRoll`) falls back to `postRollToChat`; failures are print-only (no chat notice) since there is no expression the dice can mismatch, and even an out-of-range refusal -- which the intercepted path chat-announces as player-actionable -- is harmless here because the fallback card still shows the result. Never auto-disables. The `complete` wrapper leaves a console breadcrumb on a confirmed value mismatch (`forcedDiceHonored == false`) -- on this path the wrapper is never re-fired with a stale forced set, so a first-completion mismatch is a real signal (old build ignoring forcedDice, or a reroll of the engine's chat entry) -- and `dmhub.Roll` returning nil is logged rather than claimed as success.
