...
- **Any failure the mod can detect falls through to the legacy path below.** The exception is a Codex build without forcedDice support: it cannot be feature-detected from Lua (hence the post-roll verification rather than an up-front capability check), and the engine silently ignores the field and rolls VIRTUAL dice, discarding the physical values. Mitigation: the roll's `complete` wrapper verifies `rollInfo.rolls` against the forced values (`DiceRollLogic.forcedDiceHonored`) and on a confirmed mismatch auto-disables `useForcedDice` with a loud chat warning that reports the discarded physical values, so typically one roll per session is affected (the in-memory disable re-arms on reload). If verification returns nil (rollInfo carried no readable dice), a once-per-load note is chatted without auto-disabling.
- **Verification runs once PER ROLL, via a local `checked` flag inside each roll's `complete` wrapper** (not a global session latch). Each roll verifies its own first completion; on a `false` result (engine ignored forcedDice) it auto-disables + warns. This must be per-wrapper, not global, for two reasons: (1) Codex's `doRerollAmend` reuses and RE-FIRES the initial roll's `complete` wrapper with the reroll's `rollInfo` while the closure still holds the initial forced set -- re-checking would compare the reroll's dice against the wrong set and spuriously auto-disable (the reroll false-positive bug); the wrapper's `checked` flag makes that re-fire inert. (2) `forcedDiceHonored` is a value comparison, so a virtual roll on an unsupported build can coincidentally match the forced values (guaranteed possible for a lone low-face die). A *global* latch would let one such coincidence permanently trust a broken build; per-roll verification instead re-checks every new roll, so a coincidence only delays detection to the next roll. The reroll branch installs no wrapper of its own. (The only global forcedDice notice state is `warnedUnverifiedForcedDice` for the `nil`/unverifiable case, re-armed on connect and `/dv forceddice on`.)
...
d3 forcing works because d3 is first-class in Codex's Draw Steel UI: the dice panel tile rolls `dmhub.Roll{numDice = 1, numFaces = 3}` and renders it on the d6 geometry showing 1-3 (see `Draw Steel UX Update/DicePanel.lua`), and rollable tables offer `1d3`. The DiceVision chat card mirrors that choice: `CreateDiePanel` renders `faces = 3` on the d6 icon (no d3 icon exists). Because no official code path forces a d3 and whether `rollInfo.rolls` reports such a die as 3- or 6-faced is unverifiable from Lua, `forcedDiceHonored` accepts either face count for a forced d3 entry (the result match still catches virtual rolls 2/3 of the time) rather than risking a deterministic auto-disable on every 1d3 roll.
