...
- **Verification is a ONE-TIME capability latch** (`DiceVision.forcedDiceVerified`), not a per-roll check. forcedDice support is a build property, so the first honored roll proves it and latches verification off; it re-arms on connect and on `/dv forceddice on`. This is required for correctness, not just efficiency: Codex's `doRerollAmend` reuses and RE-FIRES the initial roll's `complete` wrapper with the reroll's `rollInfo`, while the wrapper's closure still holds the initial forced set. Without the latch, that re-fire compared the reroll's dice against the initial set and spuriously auto-disabled the feature on a working build (the reroll false-positive bug). The reroll path itself installs no wrapper; the latch makes the re-fire inert.
