...
4. `handlePendingRoll()` processes result and calls `dmhub.Roll()` via one of two paths:
   - **forcedDice path** (`/dv forceddice on`, requires a Codex build where `dmhub.Roll` accepts a `forcedDice` table): passes the intact roll expression plus `forcedDice = {{numFaces, result}, ...}`; the engine computes boons/banes/tier/nats natively. Falls back to legacy per-roll on any failure the mod can detect (unparseable expression, dice count/type mismatch, out-of-range value). On a Codex build without forcedDice support the engine silently ignores the field and rolls virtual dice; the mod verifies each completed roll against the forced values and auto-disables the toggle with a chat warning on a confirmed mismatch.
